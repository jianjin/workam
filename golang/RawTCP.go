...
func latency(localAddr string, remoteHost string, port uint16) time.Duration {
	...
	log.Printf("Latency between local %v and remote %v ", localAddr, remoteAddr)
	go func() {
...
func interfaceAddress(ifaceName string) net.Addr {
	...
	log.Printf("Available addresses : 0%v", addrs)
	return addrs[1]
}
...
func sendSyn(laddr, raddr string, port uint16) time.Time {
	...
	sendTime := time.Now()
	log.Printf("Writing data %v", data)
	...
	log.Printf("Closing connection %v", raddr)
	conn.Close()
...
func receiveSynAck(localAddress, remoteAddress string) time.Time {
	...
	log.Printf("Listening %v", netaddr)
