...
func (s *scanner) scan() error {
	...
	for {
		...
		} else if tcpSYNACK, ok := tcpLayer.(*layers.TCP); !ok {
			// We panic here because this is guaranteed to never
			// happen.
			panic("tcp layer is not tcp layer :-/")
		} else if tcpSYNACK.DstPort != 54321 {
			// log.Printf("dst port %v does not match", tcp.DstPort)
		} else if tcpSYNACK.RST {
			log.Printf("  port %v closed", tcp.SrcPort)
		} else if tcpSYNACK.SYN && tcpSYNACK.ACK {
			log.Printf("received SYN&ACK.")
			tcpAck := layers.TCP{
				SrcPort: 54321,
				DstPort: 2049, // will be incremented during the scan
				ACK:     true,
				Ack:     tcpSYNACK.Seq + 1,
				Seq:     tcp.Seq + 1,
			}
			log.Printf("Sending ACK back %+v ", tcpAck)
			s.send(&eth, &ip4, &tcpAck);
